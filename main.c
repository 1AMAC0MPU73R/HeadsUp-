...
void TWI_INIT()
{
	...
	TWBR = 2;										// 50 kHz SCL, for 1 MHz internal clock
	TWSR &= ~(1 << TWPS1) | (1 << TWPS0);			// No prescalling
}

void TWI_StartOrRestart()
{
...
void TWI_WaitForFlagToBeSet(void)
{
...
void TWI_TransmitData(uint8_t TwiData, uint8_t StatusCode)
{
	...
	TWI_WaitForFlagToBeSet();
//	TWI_CheckStatusRegister(TW_MT_SLA_ACK);		// Check the SR for SLA+W trans and ACK recieved
...
void Show(unsigned char *text)
{
	...
	TWI_TransmitData(SLA,TW_MT_SLA_NACK);
	TWI_TransmitData(Datasend,TW_MT_SLA_NACK);
	for(n=0;n<16;n++){
		TWI_TransmitData(*text,TW_MT_SLA_NACK);
		++text;
		}
	TWI_STOP();
}
...
void nextline(void)
{
	TWI_StartCommunication();
	TWI_TransmitData(SLA,TW_MT_SLA_NACK);
	TWI_TransmitData(Comsend,TW_MT_SLA_NACK);
	TWI_TransmitData(Line2,TW_MT_SLA_NACK);
	TWI_STOP();
}
...
void CGRAM(void)
{
 	TWI_StartCommunication();
	TWI_TransmitData(SLA,TW_MT_SLA_NACK);
	TWI_TransmitData(Comsend,TW_MT_SLA_ACK);;
	TWI_TransmitData(0x38,TW_MT_SLA_NACK);		//go to instructino table 0
	TWI_TransmitData(0x40,TW_MT_SLA_NACK);		//Set CGRAM address to 0x00
	TWI_STOP();
	_delay_ms(10);
	...
	TWI_TransmitData(SLA,TW_MT_SLA_NACK);
	TWI_TransmitData(Comsend,TW_MT_SLA_NACK);
	TWI_TransmitData(0x00,TW_MT_SLA_NACK);		//write to first CGRAM address
	TWI_TransmitData(0x1E,TW_MT_SLA_NACK);
	TWI_TransmitData(0x18,TW_MT_SLA_NACK);
	TWI_TransmitData(0x14,TW_MT_SLA_NACK);
	TWI_TransmitData(0x12,TW_MT_SLA_NACK);
	TWI_TransmitData(0x01,TW_MT_SLA_NACK);
	TWI_TransmitData(0x00,TW_MT_SLA_NACK);
	TWI_TransmitData(0x00,TW_MT_SLA_NACK);		//8 bytes per character
...
void init_LCD()
{
	...
	TWI_TransmitData(SLA,TW_MT_SLA_NACK);
	TWI_TransmitData(Comsend,TW_MT_SLA_NACK);
	TWI_TransmitData(0x38,TW_MT_SLA_NACK);
	_delay_ms(100);
	TWI_TransmitData(0x39,TW_MT_SLA_NACK);
	_delay_ms(100);
	TWI_TransmitData(0x14,TW_MT_SLA_NACK);
	TWI_TransmitData(0x78,TW_MT_SLA_NACK);
	TWI_TransmitData(0x5E,TW_MT_SLA_NACK);
	TWI_TransmitData(0x6D,TW_MT_SLA_NACK);
	TWI_TransmitData(0x0C,TW_MT_SLA_NACK);
	TWI_TransmitData(0x01,TW_MT_SLA_NACK);
	TWI_TransmitData(0x06,TW_MT_SLA_NACK);
	_delay_ms(100);
	TWI_STOP();
